...
def make_series(growth_wk=0.004, pullback_at=None, pullback_depth=0.0,
    ...
    return pd.DataFrame({"Close": close, "Low": low, "Volume": volume}, index=idx)


def make_bounce(n=N, base_g=0.0015, dip_start=10, dip_len=3, dip_g=-0.008,
                rally_g=0.009, vol_ratio=0.75):
    """Gentle uptrend -> short dip that wicks the 50w line -> recovery rally.

    Shape matters: the dip must be SHORT. A deep multi-month correction turns
    the 26-week regression negative and the ratio gate then (correctly)
    rejects it -- see the "deep correction" case below.
    """
    idx = pd.date_range("2020-01-06", periods=n, freq="W-MON")
    close = np.zeros(n); px = 50.0; ds = n - 1 - dip_start
    for i in range(n):
...
# Every tier must be reachable, otherwise the tiering is decorative.
tiers = set()
for ds in range(2, 40):
    r = fs.evaluate_ticker("T", make_bounce(dip_start=ds))
    if r: tiers.add(r["tier"])
check("Fresh and Further Along tiers are both reachable",
      {fs.TIER_FRESH, fs.TIER_FURTHER} <= tiers, f"reached: {sorted(tiers)}")
...
worst = dict(close=100.0, accel_3w=-10.0, macd_delta_3w=0.01, slope_ratio=0.1,
             dist_200w=300.0, vol_ratio=1.09)
