...
def _render_single_fund(sym: str):
    ...
    st.markdown(
        ...
        f'{"<span style=\\"color:#F97316;margin-left:8px;font-size:10px\\">CIT — proxy data " + proxy + "</span>" if f["is_cit"] else ""}'
        f'</div>'
        f'</div>'
        ...
        f'<div style="font-size:16px;font-weight:700;color:{"#22C55E" if exp_pct and float(exp_pct if float(exp_pct)>0.2 else float(exp_pct)*100) < 0.20 else _GOLD}">{exp_str}</div></div>'
